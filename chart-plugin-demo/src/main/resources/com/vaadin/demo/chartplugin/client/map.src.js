 ...
 * 
 * Changes:
 * 1/17/2013: Changes in getBox for Vaadin Charts test case. 
 * 			  Paths are read directly form the world-map-shapes.js by a country code.
 * 
 */
 ...
 */
 (function(Highcharts) {
	...
	Highcharts.seriesTypes.map = Highcharts.extendClass(Highcharts.seriesTypes.scatter, {
		...
		getBox: function() {
			...
			each(this.options.data, function(point) {
				...
				// Lines added for Vaadin Charts Test case ------
				if(shapes[point.name] == null) {
					return;
				}
				point.path = Highcharts.pathToArray(shapes[point.name]);
				// ----------------------------------------------
	...
	Highcharts.Map = function (options, callback) {
		...
		console.log('--- calling new Highcharts.Chart(options, callback) ---');
		return new Highcharts.Chart(options, callback);
	};
	
})(Highcharts);window.console && console.log('--- map.src.js runs! ---');