...
public class VTimelineWidget extends FocusPanel implements Paintable,
    ...
    private final List<HTML> legendValues = new ArrayList<HTML>();
     ...
     *            The chart mode
     */
    private void setChartMode(PlotMode mode, boolean updateToServer) {
        if (mode != null) {
            display.setChartMode(mode, true);
            browser.setChartMode(mode, true);
            if (updateToServer) {
            ...
            if (!captions.isEmpty()) {
                ...
                for (Integer graph : captions.keySet()) {
                    ...
                    if (colors.get(graph) != null) {
                        HTML color = new HTML("<b class='"
                                + CLASSNAME_LEGENDLABEL + "'>—</b>");
                        color.setStyleName(CLASSNAME_LEGEND + "-slot");
                        color.setStyleName(CLASSNAME_LEGEND + "-slot-color");
                    ...
                    HTML html = new HTML();
                    html.setStyleName(CLASSNAME_LEGEND + "-slot");
                    html.addStyleName(CLASSNAME_LEGEND + "-slot-value");
                    legend.add(html);
    ...
    /**
     * Removed all values from the legend
     */
    public void clearLegendValue() {
        for (HTML lbl : legendValues) {
            lbl.setHTML("&nbsp;");
        }
    }
