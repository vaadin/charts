...
private Chart getWeatherChart(ChartsData data) {
  ...
  conf.getChart().setType(ChartType.LINE);
  DataProvider<WeatherInfo,?> dataProvider = new ListDataProvider<>(data.getWeatherData());
...
In this case, the value returned from a call to the [methodname]#getDate()# method in [classname]#WeatherInfo# will be used as the value of X for each data point.
...
[source,java]
----
DataProvider<WeatherInfo,?> dataProvider = new ListDataProvider<>(weatherInfo);
...
//////////////////////////////////////////
Commented due to no filter in DataProvider yet
