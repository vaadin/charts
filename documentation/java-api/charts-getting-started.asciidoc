...
You can then be import the project into your IDE. You can use the IDE to generate a new maven project using the `vaadin-archetype-application` archetype, version 7.3.6, but in the following we do it on command-line.
...
[prompt]#+++$+++# [command]#mvn# archetype:generate -DarchetypeGroupId=com.vaadin \
 ...
 -DarchetypeVersion=[replaceable]#7.3.6#
----
