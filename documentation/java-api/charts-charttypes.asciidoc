...
<<charts-data#charts-data.dataseries, "Chart Data">> for more details. A data
...
When using [classname]#ContainerSeries#, you need to specify the property IDs
of the container properties containing OHLC values:
[methodName]#setXPropertyId()#, [methodName]#setOpenPropertyId()#,
[methodName]#setClosePropertyId()# [methodname]#setHighPropertyId()#, and
[methodName]#setLowPropertyId()#.
