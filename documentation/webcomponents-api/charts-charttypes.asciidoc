...
<script>
  (function () {
    ...
    const scatter = document.querySelector('#scatterExample');
    ...
    function createRandomPoint() {
      ...
      const z = Math.cos(lng) * Math.cos(lat);
      const colorLevel = parseInt(Math.round((1 - z) * 127));
