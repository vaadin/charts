 ...
 * <p>
 * All configuration and data is given through a {@link Configuration} object. Here is a simple usage example:
 * <p>
 * <code><pre>
 ...
 * </pre></code>
 * <p>
 * Unless otherwise documented, dynamic changes to the chart configuration are not immediately reflected to an already drawn component. To redraw the chart you
 * should call {@link #drawChart()} or {@link #drawChart(Configuration)}.
 * <p>
 * The implementation relies on the <a href="http://www.highcharts.com/">HighCharts JS</a> library. Developers can also use its raw JS API via the
 * {@link #drawChart(String)} method.
 * <p>
...
public class Chart extends AbstractComponent {
  ...
  private final class ChartServerRpcImplementation implements ChartServerRpc {
    ...
    public void onChartClick(final double x, final double y, final int offsetX, int offsetY) {
      final ChartClickEvent chartClickEvent = new ChartClickEvent(Chart.this, x, y, offsetX, offsetY);
      fireEvent(chartClickEvent);
    }

    @Override
    public void onPointClick(final double x, final double y, final int seriesIndex, final String category, final int pointIndex, final int absoluteX, final int absoluteY) {

      final Series series = getSeriesBasedOnIndex(seriesIndex);
      final PointClickEvent pointClickEvent = new PointClickEvent(Chart.this, x, y, series, category, pointIndex, absoluteX, absoluteY);
    ...
    @Override
    public void onSelection(final double selectionStart, final double selectionEnd, final double valueStart, final double valueEnd) {
      final ChartSelectionEvent selectionEvent = new ChartSelectionEvent(Chart.this, selectionStart, selectionEnd, valueStart, valueEnd);
      fireEvent(selectionEvent);
    }

    @Override
    public void onLegendItemClick(final int seriesIndex, int seriesItemIndex) {
      final LegendItemClickEvent itemClickEvent = new LegendItemClickEvent(Chart.this, getSeriesBasedOnIndex(seriesIndex), seriesItemIndex);
   ...
   * <p>
   * In charts with multiple series, the type can also be defined by setting series specific plot options.
   * 
   * @see #Chart()
  ...
  public void beforeClientResponse(boolean initial) {
    ...
    if(initial || stateDirty) {
      getState().confState = configuration == null ? null : configuration.toString();
  ...
  /**
   * Draws chart with the given configuration. The configuration is given in Highcharts configuration format (JavaScript) and it overrides settings done via the
   * standard {@link Configuration} object.
   * <p>
   * Although using strictly typed Java API (via {@link #getConfiguration()} is highly encouraged, using the low level Highcharts configuration may be handy in
   * some occasions.
   * <p>
   * Note, that this configuration is evaluated as JavaScript and the config may contain JavaScript functions. Thus developers should pay attention to data that
   * is passed to this function.
   * <p>
   * Note, that if further modifications are done to the configuration, the method must be called again to redraw the UI.
   * 
   ...
   * <p>
   * Note that if you modify the underlying {@link Series} directly the chart will automatically be updated to reflect this unless explicitly told not to. The
   * methods listed below can be used as an example.
   * 
   * @see DataSeries#add(com.vaadin.addon.charts.model.DataSeriesItem)
   * @see DataSeries#addData(com.vaadin.addon.charts.model.DataSeriesItem, boolean)
  ...
  private final static Method chartClickMethod = ReflectTools.findMethod(ChartClickListener.class, "onClick", ChartClickEvent.class);

  private final static Method pointClickMethod = ReflectTools.findMethod(PointClickListener.class, "onClick", PointClickEvent.class);

  private final static Method chartSelectionMethod = ReflectTools.findMethod(ChartSelectionListener.class, "onSelection", ChartSelectionEvent.class);

  private final static Method legendItemClickMethod = ReflectTools.findMethod(LegendItemClickListener.class, "onClick", LegendItemClickEvent.class);
  ...
  public void addChartClickListener(ChartClickListener listener) {
    this.addListener(ChartConnector.CHART_CLICK_EVENT_ID, ChartClickEvent.class, listener, chartClickMethod);
  }
  ...
  public void removeChartClickListener(ChartClickListener listener) {
    this.removeListener(ChartConnector.CHART_CLICK_EVENT_ID, ChartClickEvent.class, listener);
  }
  ...
  public void addPointClickListener(PointClickListener listener) {
    this.addListener(ChartConnector.POINT_CLICK_EVENT_ID, PointClickEvent.class, listener, pointClickMethod);
  }
  ...
  public void removePointClickListener(PointClickListener listener) {
    this.removeListener(ChartConnector.POINT_CLICK_EVENT_ID, PointClickEvent.class, listener);
  }
   ...
   * <br />
   * 
   * Note that if a chart selection listener is set, default action for selection is prevented. Most commonly this means that client side zoom doesn't work and
   * you are responsible for setting the zoom, etc in the listener implementation.
   * 
   * @param listener
   */
  public void addChartSelectionListener(ChartSelectionListener listener) {
    this.addListener(ChartConnector.CHART_SELECTION_EVENT_ID, ChartSelectionEvent.class, listener, chartSelectionMethod);
  }
  ...
  public void removeChartSelectionListener(ChartSelectionListener listener) {
    this.removeListener(ChartConnector.CHART_SELECTION_EVENT_ID, ChartSelectionEvent.class, listener);
  }

  /**
   * Adds a legend item click listener, which will be notified of clicks on the legend's items
   * 
   * @param listener
   */
  public void addLegendItemClickListener(LegendItemClickListener listener) {
    this.addListener(ChartConnector.LEGENDITEM_CLICK_EVENT_ID, LegendItemClickEvent.class, listener, legendItemClickMethod);
  }
  ...
  public void removeLegendItemClickListener(LegendItemClickListener listener) {
    this.removeListener(ChartConnector.LEGENDITEM_CLICK_EVENT_ID, LegendItemClickEvent.class, listener);
  }

  /**
   * Listens to events on the series attached to the chart and redraws as necessary.
   */
  private final ConfigurationMutationListener dataSeriesEventListener = new ConfigurationMutationListener() {
    ...
    public void dataAdded(DataAddedEvent event) {
      if(event.getItem() != null) {
        if(event.getItem().getX() != null) {
          // x,y type data
          getRpcProxy(ChartClientRpc.class).addPoint(event.getItem().toString(), getSeriesIndex(event), event.isShift());
        }
      }
    }
    ...
    public void dataRemoved(DataRemovedEvent event) {
      getRpcProxy(ChartClientRpc.class).removePoint(event.getIndex(), getSeriesIndex(event));
    }

    @Override
    public void dataUpdated(DataUpdatedEvent event) {
      if(event.getValue() != null) {
        getRpcProxy(ChartClientRpc.class).updatePointValue(getSeriesIndex(event), event.getPointIndex(), event.getValue().doubleValue());
      }
      else {
        getRpcProxy(ChartClientRpc.class).updatePoint(getSeriesIndex(event), event.getPointIndex(), event.getItem().toString());
      }
    }

    @Override
    public void seriesEnablation(SeriesEnablationEvent event) {
      getRpcProxy(ChartClientRpc.class).setSeriesEnabled(getSeriesIndex(event), event.isEnabled());
    }
   ...
   * <p>
   * Note, that calling this method on already displayed component don't necessary update it. Developer should call {@link #drawChart()} or
