...
public class Canvas extends FocusWidget {
     ...
     * used to apply additional style rules to the canvas widget.
     * 
     * @param width the width of the canvas drawing area in CSS units
     * @param height the height of the canvas drawing area in CSS units
     */
     ...
     * 
     * @param width the width of the drawing area, in CSS units (e.g. "100px")
     */
    @Override
     ...
     * border, margin, and padding.
     * 
     * @param height the height of the drawing area, in CSS units (e.g. "100px")
     */
    @Override
     ...
     * using hexadecimal numbers. If you want to make the background
     ...
     * <code>Canvas.TRANSPARENT</code> if a transparent background is needed
     */
     ...
     * <p>
     * Use {@link Gradient#addColorStop(double, String)} to add colors
     * and offsets to a gradient. The 0 offset in this case is the start of
     * the gradient <code>(x0, y0)</code> while the 1 offset is the end of the
     * gradient <code>(x1, y1)</code>.
     * <p>
     * The returned object can be assigned to the
     * {@link #setFillStyle(Gradient)} and
     * {@link #setStrokeStyle(Gradient)} methods.
     * 
     * @param x0
     * @param y0
     * @param x1
     * @param y1
     * @return an object that represents a linear gradient between the two
     * input coordinates
     */
     ...
     * <p>
     * Use {@link Gradient#addColorStop(double, String)} to add colors
     * and offsets to a gradient. The 0 offset in this case is the circumference
     * of the first circle <code>(x0, y0, r0)</code> while the 1 offset is the
     ...
     * @param y1
     * @param r1
     * @return an object that represents a radial gradient between the edges
     * of two circles using the input coordinates and the radii
     */
    ...
    public void drawImage(Image image, double x, double y) {
        if (image != null && image.isLoaded()) {
            ...
            impl.drawImage(image.getElement(), 0, 0, width, height, x, y, width, height);
        }
    }
    ...
    public void drawImage(Image image, double x, double y, double width, double height) {
        if (image != null && image.isLoaded()) {
            impl.drawImage(image.getElement(), 0, 0, image.getWidth(), image.getHeight(), x, y, width, height);
        }
    }
    ...
    public void drawImage(Image image, double sx, double sy, double sWidth, double sHeight,
        ...
        if (image != null && image.isLoaded()) {
            impl.drawImage(image.getElement(), sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight);
        }
    }
     ...
     * default, this parameter’s value is 1.0 (no transparency).
     * <p>
     * The canvas uses the alpha value in the current graphics state to
     * determine how to composite newly painted objects.
     * 
     * @param globalAlpha the global alpha channel value (range 0.0 to 1.0)
     */
    ...
    /**
     * See setter method {@link #setGlobalAlpha(double)} for a fully
    ...
    /**
     * Determines how the canvas contents are displayed relative to any
     * existing content. The string identifies the desired compositing mode.
     * If you do not set this value explicitly, the canvas uses the
     * <code>Canvas.SOURCE_OVER</code> compositing setting by default.
     * 
     * @param globalCompositeOperation <code>Canvas.SOURCE_OVER</code>
     * or <code>Canvas.DESTINATION_OVER</code>
     */
    ...
    /**
     * See setter method {@link #setGlobalCompositeOperation(String)}
     ...
     * 
     * @param strokeStyle a String representing the stroke color
     */
     ...
     * property, the canvas sets the fill style parameter of the graphics
     * state.
     * <p>
     ...
     * 
     * @param fillStyle a Gradient representing the fill style
     */
     ...
     * which determines the opacity of the color.
     * 
     * @param fillStyle a String representing the fill color
     */
     ...
     * {@link #setFillStyle(Gradient)} for a fully detailed
     * description.
     * 
    ...
    /**
     * See setter method {@link #setLineWidth(double)} for a fully
     * detailed description.
     * 
     ...
     * 
     * @param lineCap <code>Canvas.BUTT</code>, <code>Canvas.ROUND</code> or
     ...
     * 
     * @param lineJoin <code>Canvas.ROUND</code>, <code>Canvas.BEVEL</code> or
    ...
    /**
     * See setter method {@link #setLineJoin(String)} for a fully
     * detailed description.
     * 
    ...
    /**
     * See setter method {@link #setMiterLimit(double)} for a fully
     * detailed description.
     * 
